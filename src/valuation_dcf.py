...
def convert_rate(previous_rate, previous_periodicity=1, new_periodicity=1):
    """Convert a rate between different compounding periodicities.

    Parameters
    ----------
    previous_rate : float
        Rate expressed with ``previous_periodicity`` compounding periods.
    previous_periodicity : int, optional
        Number of periods used for ``previous_rate``. Defaults to ``1``.
    new_periodicity : int, optional
        Desired number of compounding periods. Defaults to ``1``.

    Returns
    -------
    float
        Equivalent rate with the new periodicity.
    """
...
def freq_str_to_months(freq_str: str) -> int:
    """Convert a frequency string into number of months.

    Parameters
    ----------
    freq_str : str
        Frequency string (``"Y"``, ``"2Q"``, ``"Q"`` or ``"M"``).

    Returns
    -------
    int
        Number of months corresponding to the frequency.
    """
    if freq_str == "Y":
        freq = 12
    elif freq_str == "2Q":
...
def freq_months_to_freq_str(no_months: int) -> str:
    """Convert a number of months into a frequency string.

    Parameters
    ----------
    no_months : int
        Number of months (1, 3, 6 or 12).

    Returns
    -------
    str
        Frequency string (``"M"``, ``"Q"``, ``"2Q"`` or ``"Y"``).
    """
    if no_months == 12:
        freq_str = "Y"
    elif no_months == 6:
...
class LineBuilder:
    ...
    def __neg__(self):
        ...
        def cf_fun(
            start_date=None, end_date=None, nper=None, freq=None, initial_period=False
        ):
            cf = -self._get_line(
            ...
            if breakdown_level == 0:
                ...
                if self.ignore_last_line:
                    ...
                    table1 = self.get_table(
                        ...
                        freq=freq,
                        initial_period=initial_period,
                        breakdown_level=breakdown_level - 1,
                    )

                if other.ignore_last_line:
                    table2 = other.get_table(
                        start_date=start_date,
                        end_date=end_date,
                        nper=nper,
                        freq=freq,
    ...
    def _get_line_bak(
        self, start_date=None, end_date=None, nper=None, freq=None, initial_period=False
    ):
        ...
        cols = pd.date_range(
            start=start_date, end=end_date, periods=nper + 1, freq=freq
        )
        ...
        if self.tenor is not None:
            if self.effective_date is not None:
                initial_zeros = np.sum(
                    cols < self.effective_date
                )  # number of dates before the effective one
                if (
                    initial_zeros < n
                ):  # there are dates in the range before the effective one
                    # or the effective date is out of the range (in the past)
                    temp_date = cols[cols >= self.effective_date][
                        0
                    ]  # first date of the range bigger than
                    # or equal to the effective one
                    initial_coverage = (
                        temp_date.month - self.effective_date.month + 1
                    ) / freq
            ...
            if initial_coverage != 0:
                ...
                left_over_periods = int(left_over_periods) + int(
                    left_over_periods % 1 != 0
                )  # ceiling
                if (
                    left_over_periods <= n - initial_zeros - 1
                ):  # the left over periods fit in the array
                ...
                multiplier = np.concatenate(
                    (multiplier, np.zeros(n - 1 - left_over_periods - initial_zeros))
                )
    ...
    def _get_line(
        ...
        if self.tenor is not None:
            ...
            if (
                initial_zeros == 0
            ):  # the effective date is out of the range (in the past) or it's the 1st element
                expired_dates = pd.date_range(
                    start=self.effective_date, end=start_date2, freq=freq_str
                )
                ...
                left_over_periods = max(
                    self.tenor * 12 / freq - expired_dates.size - initial_coverage, 0
                )
                left_over_periods = int(left_over_periods) + int(
                    left_over_periods % 1 != 0
                )  # ceiling
                ...
                multiplier[-1] = final_coverage
                multiplier = np.concatenate(
                    (multiplier, np.zeros(n - 1 - left_over_periods))
                )
                # multiplier = left_over_periods + remaining periods
            elif (
                0 < initial_zeros & initial_zeros < n
            ):  # there are dates in the range before the effective one
                temp_date = cols[cols >= self.effective_date][
                    0
                ]  # first date of the range bigger than
...
class Immutable(LineBuilder):
    def __init__(self, name, value=np.nan, effective_date=None, tenor=None):
        LineBuilder.__init__(
            self, name=name, effective_date=effective_date, tenor=tenor
        )
        self.value = value
...
class Flator(LineBuilder):
    ...
    def __mul__(self, other):
        try:
            ...
            def _line_fun(
                ...
                deflators = self.get_multipliers(
                    start_date=start_date,
                    end_date=end_date,
                    nper=nper,
                    freq=freq,
    ...
    def get_multipliers(
        ...
        base_date = (
            self.base_date if hasattr(self, "base_date") else deflators.columns[0]
        )
    ...
    def __init__(
        ...
        if effective_date is not None:
            ...
            if self.nper is not None:
                if self.freq_str == "2Q":
                    self.final_date = (
                        self.effective_date + 2 * self.nper * pd.offsets.QuarterEnd()
                    )
                else:
                    self.final_date = pd.date_range(
                        start=self.effective_date, periods=self.nper + 1, freq=freq_str
                    )[-1]
            else:
            ...
            self.final_date = None
        LineBuilder.__init__(
            self, name=name, effective_date=effective_date, tenor=self.tenor
        )
    ...
    def __mul__(self, other):
        try:
            ...
            def line_fun(
                ...
                quantities = self.get_multipliers(
                    start_date=start_date,
                    end_date=end_date,
                    nper=nper,
                    freq=freq,
...
class CashFlow(Aggregatable):
    ...
    def __mul__(self, other):
        try:
            ...
            def line_fun(
                ...
                multipliers = other.get_multipliers(
                    start_date=start_date,
                    end_date=end_date,
                    nper=nper,
                    freq=freq,
    ...
    def get_dcf(
        ...
        if deflator is None and discount_rate is not None:
            ...
            effective_disc_rate = convert_rate(
                discount_rate, previous_periodicity=1, new_periodicity=new_periodicity
            )
    ...
    def get_npv(
        ...
        npv = pd.DataFrame(
            dcf.sum(axis=1).rename({self.name: f"{self.name} NPV"}),
            columns=[pd.to_datetime(start_date)],
        )
...
class FixedRateLoan(CashFlow):
    def __init__(
        ...
        if self.interest_rate == 0:
            ...
            payment = (
                self.principal * i * (1 + i) ** self.nper / ((1 + i) ** self.nper - 1)
            )
        self.value = payment
...
class EBITDA(dict):
    ...
    def get_table(
        ...
        if "EBITDA Margin" not in table.index:
            margin_df = margin.get_multipliers(
                start_date=start_date,
                end_date=end_date,
                nper=nper,
                freq=freq,
...
class Enterprise(dict):
    ...
    def __init__(
        ...
        if (
            ...
            operating_revenues is not None
            ...
            raise ValueError(
                "Only either an EBITDA or a combination of OpRev and OpEx must given."
            )
