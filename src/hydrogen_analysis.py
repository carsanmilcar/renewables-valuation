...
def LCvsElectrolyzerANDElectricityPrice(
    project, generalSpecs, capex_limits, electricity_price_limits, inf_time_series
):
    """Plot LCOH versus electrolyzer CAPEX and electricity price.
    ...
    generalSpecs : general
        General specifications with utility prices.
    capex_limits : array-like
        Minimum and maximum electrolyzer CAPEX in euros per MW.
    electricity_price_limits : array-like
        Minimum and maximum electricity price in euros per MWh.
    inf_time_series : array-like
        Inflation factors for the analysis period.

    Returns
    -------
    matplotlib.figure.Figure
        Generated contour figure.
    pandas.DataFrame
        Table of levelized cost values.
    """
    ...
    power = (
        project.main_product.capacity
        * project.feedstock_dict["Electricity"].usage
        * 1000
        / 24
    )
    print(power)
    ...
    electricity_price_range = np.linspace(
        electricity_price_limits[0], electricity_price_limits[1], 20
    )
    ...
    Z_df = np.zeros(
        (
            int(np.floor(len(electricity_price_range) / 4)),
            int(np.floor(len(capex_range) / 4)),
        )
    )
    ...
    for cap_idx in range(len(capex_range)):
        for el_idx in range(len(electricity_price_range)):
            ...
            project.capex_dict["Stack Replacements"].amount = (
                capex_range[cap_idx] * 0.45
            )
            generalSpecs.utilities_and_materials_dict[
                "Electricity"
            ].price = electricity_price_range[el_idx]
            ...
            if el_idx % 4 == 0 and cap_idx % 4 == 0:
                Z_df[Z_df.shape[0] - 1 - int(np.floor(el_idx / 4))][
                    int(np.floor(cap_idx / 4))
                ] = np.round(Z[el_idx][cap_idx], 2)
                x_df[int(np.floor(cap_idx / 4))] = np.round(
                    capex_range[cap_idx] / power, 2
                )
                y_df[Z_df.shape[0] - 1 - int(np.floor(el_idx / 4))] = np.round(
                    electricity_price_range[el_idx], 2
                )
    ...
    ax.plot(
        original_electrolyzer_cost / power,
        original_el_price,
        ls="None",
...
def LCvsCapexANDElectricityPrice(
    project, generalSpecs, capex_limits, electricity_price_limits, inf_time_series
):
    """Analyze LCOH versus total CAPEX and electricity price.
    ...
    generalSpecs : general
        General specifications with utility prices.
    capex_limits : array-like
        Lower and upper bounds for initial CAPEX in euros.
    electricity_price_limits : array-like
        Lower and upper bounds for electricity price in euros per MWh.
    inf_time_series : array-like
        Inflation factors for the analysis period.
    ...
    project.capex_dict = {}
    project.addCost(
        "Capex", "capex", 8000000, project.start_time, project.start_time, -1, 1, 0, 0
    )
    project.capex_dict["Capex"].setDepreciation(20, 0)

    replacement_time = int(
        np.ceil(80000 / (project.main_product.load_factor * 365 * 24))
    )
    project.addCost(
        "Stack replacements",
        "capex",
        0.45 * 8000000,
        project.start_time + replacement_time,
        project.end_time,
        replacement_time,
        1,
        0,
...
def LCvsElectricityPriceANDLoadFactor(
    project, generalSpecs, load_limits, electricity_price_limits, inf_time_series
):
    """Plot LCOH versus electricity price and plant load factor.
    ...
    load_limits : array-like
        Minimum and maximum plant load factor as fractions.
    electricity_price_limits : array-like
        Minimum and maximum electricity price in euros per MWh.
    inf_time_series : array-like
        Inflation factors for the analysis period.
    ...
    for load_idx in range(len(load_range)):
        for el_idx in range(len(electricity_price_range)):
            ...
            project.main_product.yearly_output = (
                load_range[load_idx] * 365 * project.main_product.capacity
            )
            generalSpecs.utilities_and_materials_dict[
                "Electricity"
            ].price = electricity_price_range[el_idx]
            ...
            if el_idx % 4 == 0 and load_idx % 4 == 0:
                Z_df[Z_df.shape[0] - 1 - int(np.floor(el_idx / 4))][
                    int(np.floor(load_idx / 4))
                ] = np.round(Z[el_idx][load_idx], 2)
                x_df[int(np.floor(load_idx / 4))] = np.round(
                    load_range[load_idx] * 100, 2
                )
                y_df[Z_df.shape[0] - 1 - int(np.floor(el_idx / 4))] = np.round(
                    electricity_price_range[el_idx], 2
                )
...
def LCSpiderChart(project, generalSpecs, inf_time_series, tax_rate):
    """Create a spider chart showing LCOH sensitivity.
    ...
    el_price_range = (
        ...
        * generalSpecs.utilities_and_materials_dict["Electricity"].price
    )
    y_el_price = np.linspace(lower, upper, length)

    electrolyzer_range = (
        np.linspace(lower, upper, length) * project.capex_dict["Electrolyzer"].amount
    )
    ...
    for idx in range(len(dr_range)):
        ...
        generalSpecs.utilities_and_materials_dict["Electricity"].price = el_price_range[
            idx
        ]
        project.addFeedstockCosts(generalSpecs.utilities_and_materials_dict)
        ...
        project.main_product.yearly_output = (
            load_range[idx] * 365 * project.main_product.capacity
        )
        project.addFeedstockCosts(generalSpecs.utilities_and_materials_dict)
    ...
    ax.plot(
        100,
        project.getLC(tax_rate, inf_time_series),
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def NPVvsPrice(project, price_limits, inf_time_series):
    """Plot NPV versus hydrogen selling price.

    Parameters
    ----------
    project : project
        Project instance to analyze.
    price_limits : array-like
        Minimum and maximum hydrogen selling price.
    inf_time_series : array-like
        Inflation factors for the analysis period.
    ...
    -------
    matplotlib.figure.Figure
        Figure showing the NPV variation with price.
    """
    ...
    ax.plot(
        original_price,
        project.getNPV(0.3, inf_time_series) / 1000000,
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def IRRvsPrice(project, price_limits, inf_time_series):
    """Plot IRR versus hydrogen selling price.
    ...
    -------
    matplotlib.figure.Figure
        Figure showing the IRR variation with price.
    """
    ...
    ax.plot(
        original_price,
        project.getIRR(0.3, inf_time_series) * 100,
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def NPVvsPriceAndDiscountRate(
    project, price_limits, dr_limits, inf_time_series, tax_rate
):
    """Plot NPV as a function of price and discount rate.
    ...
    project : project
        Project instance to analyze.
    price_limits : array-like
        Range of hydrogen selling prices.
    dr_limits : array-like
        Range of discount rates.
    inf_time_series : array-like
        Inflation factors for the analysis period.
    tax_rate : float
        Income tax rate used in calculations.

    Returns
    -------
    matplotlib.figure.Figure
        Contour figure with NPV variation.
    """
...
def NPVSpiderChart(project, generalSpecs, inf_time_series, tax_rate):
    """Create a spider chart showing NPV sensitivity.
    ...
    inf_time_series : array-like
        Inflation factors for the analysis period.
    tax_rate : float
        Income tax rate used in the analysis.
    ...
    ax.plot(
        100,
        project.getNPV(tax_rate, inf_time_series) / 1000000,
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def IRRSpiderChart(project, generalSpecs, inf_time_series, tax_rate):
    """Create a spider chart showing IRR sensitivity.
    ...
    ax.plot(
        100,
        project.getIRR(tax_rate, inf_time_series) * 100,
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def PaybackvsPrice(project, price_limits, inf_time_series):
    """Plot payback period versus hydrogen price.
    ...
    -------
    matplotlib.figure.Figure
        Figure showing payback variation with price.
    """
    ...
    ax.plot(
        original_price,
        project.getPayback(0.3, inf_time_series),
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def PaybackSpiderChart(project, generalSpecs, inf_time_series, tax_rate):
    """Create a spider chart showing payback sensitivity.
    ...
    ax.plot(
        100,
        project.getPayback(tax_rate, inf_time_series),
        markerfacecolor="orange",
        markeredgecolor="red",
        marker="o",
    )
...
def RinvvsPriceAndDiscountRate(
    project, price_limits, dr_limits, inf_time_series, tax_rate
):
    """Plot return on investment versus price and discount rate.
    ...
    -------
    matplotlib.figure.Figure
        Contour figure with ROI variation.
    """
    ...
    for price_idx in range(len(price_range)):
        for dr_idx in range(len(dr_range)):
            ...
            Z[dr_idx][price_idx] = (
                project.getReturnOnInvestment(tax_rate, inf_time_series) / 1000000
            )
...
def RinvSpiderChart(project, generalSpecs, inf_time_series, tax_rate):
    """Create a spider chart showing ROI sensitivity.
...
def exportPlots(project, generalSpecs, inflation_time_series, tax_rate):
    """Generate and export all analysis plots to files.
    ...
    generalSpecs : general
        General specifications with utility prices.
    inflation_time_series : array-like
        Inflation factors for the analysis period.
    tax_rate : float
        Income tax rate used in calculations.
    ...
    fig, tab = LCvsElectrolyzerANDElectricityPrice(
        carboneras,
        generalSpecs,
        electrolyzer_limits,
        el_price_limits,
    ...
    load_limits = [0.7, 1]
    fig, tab = LCvsElectricityPriceANDLoadFactor(
        carboneras, generalSpecs, load_limits, el_price_limits, inflation_factors
    )
    fig.savefig(directory + "/LCvsLoadAndElectricity.png")
    tab.to_csv(directory + "/LCvsLoadAndElectricity.csv")

    # LCOH Spider Graph
    LCSpiderChart(carboneras, generalSpecs, inflation_factors, tax_rate).savefig(
        directory + "/LCspider.png"
    )

    # NPV vs Price
    hy_price_limits = [2, 10]
    NPVvsPrice(carboneras, hy_price_limits, inflation_factors).savefig(
        directory + "/NPVvsPrice.png"
    )

    # NPV vs Cost of Capital and Price
    dr_limits = [0.05, 0.15]
    NPVvsPriceAndDiscountRate(
        carboneras, hy_price_limits, dr_limits, inflation_factors, tax_rate
    ).savefig(directory + "/NPVvsCostOfCapitalAndPrice.png")

    # NPV Spider Graph
    NPVSpiderChart(carboneras, generalSpecs, inflation_factors, tax_rate).savefig(
        directory + "/NPVspider.png"
    )

    # IRR vs Price
    hy_price_limits = [5, 10]
    IRRvsPrice(carboneras, hy_price_limits, inflation_factors).savefig(
        directory + "/IRRvsPrice.png"
    )

    # IRR Spider Graph
    IRRSpiderChart(carboneras, generalSpecs, inflation_factors, tax_rate).savefig(
        directory + "/IRRspider.png"
    )

    # Payback vs Price
    hy_price_limits = [5, 10]
    PaybackvsPrice(carboneras, hy_price_limits, inflation_factors).savefig(
        directory + "/PaybackvsPrice.png"
    )

    # Payback Spider Graph
    PaybackSpiderChart(carboneras, generalSpecs, inflation_factors, tax_rate).savefig(
        directory + "/Paybackspider.png"
    )

    # Rinv vs Cost of Capital and Price
    hy_price_limits = [5, 10]
    dr_limits = [0.05, 0.15]
    RinvvsPriceAndDiscountRate(
        carboneras, hy_price_limits, dr_limits, inflation_factors, tax_rate
    ).savefig(directory + "/RinvvsCostOfCapitalAndPrice.png")

    # Rinv Spider Graph
    RinvSpiderChart(carboneras, generalSpecs, inflation_factors, tax_rate).savefig(
        directory + "/Rinvspider.png"
    )
